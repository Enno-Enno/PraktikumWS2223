...
\subsubsection[]{Fallzeit der kleinen Kugel}
...
\begin{table}%[h!]
    \caption{Fallzeit der kleinen Kugel bei Zimmertemperatur}
...
\begin{table}
    \centering
    \caption{Durchschnittliche Fallzeit für die kleine Kugel}
    \label{tab:mittel_fallzeit_klein}
    \sisetup{table-format=2.2}
    \begin{tabular}[]{c S @{${}\pm{}$} S}
        \toprule
        Ausrichtung   & \multicolumn{2}{c}{$\overline{t} / \unit{\s}  $} \\
        \midrule
        oben  & \num{12.32} &  \num{0.16}    \\
        unten & \num{12.25} &  \num{0.18}    \\
        \bottomrule
