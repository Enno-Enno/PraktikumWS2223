...
Das arithmetischen Mittel der Laufzeiten $\overline{t_text{oben}}$ und $\overline{t_text{unten}}$
werden nach \eqref{eq:artim_mit} für $N= 10$ Werte aus \ref{tab:klKu_Zitemp} berechnet.
Die Standardmessfehler des Mittelwertes werden in der 
...
\begin{table}
    \centering
    \caption{Viskosität des Wassers nach Fallrichtung aufgeteilt}
    \label{tab:visk_kl_Zitemp}
    \sisetup{table-format=1.3}
    \begin{tabular}[]{c S @{${}\pm{}$} S}
        \toprule
        Ausrichtung   & \multicolumn{2}{c}{$\eta / \unit{\milli\pascal\s}  $} \\
        \midrule
        oben & \num{1.165} &  \num{0.015}    \\
        unten & \num{1.158} &  \num{0.018}    \\
        \bottomrule
