...
\begin{table}
    ...
    \begin{tabular}[]{S[table-format=2.0] S[table-format=0.5] S @{${}\pm{}$} S[table-format=1.3] S @{${}\pm{}$}  S[table-format=1.3] S[table-format=1.3]}
        \toprule
        {$T /\unit{\celsius}$} & {$\rho_{\text{Wasser}}$ \cite{geschke}} & \multicolumn{2}{c}{$ \eta / \unit{\milli\Pa\s}$} & \multicolumn{2}{c}{$ \eta / \unit{\milli\Pa\s}$} & {Vergleichswerte Geschke \cite{geschke}$ \eta / \unit{\milli\Pa\s}$}\\
