...
\begin{table}[h!]
    ...
    \begin{tabular}[]{S S[table-format=1.4] S[table-format=2.2] @{${}\pm{}$} S[table-format=0.2] S[table-format=1.4] @{${}\pm{}$} S[table-format=0.2]}
        \toprule
        {Kugel} & {$m / \unit{\kg}$} & \multicolumn{2}{c}{$d / \unit{\mm}$}  & \multicolumn{2}{c}{{$\rho / \left( \unit{\mg \per \cubic\mm}\right)$}} \\
        \midrule
...
das Python-paket \texttt{uncertainties} \cite{uncertainties}\cite{python}.
\subsection{Messreihe 1: kleine Kugel, konstante Temperatur}
