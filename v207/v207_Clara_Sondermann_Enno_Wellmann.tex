...
Die Dichte $\rho$ errechnet sich aus dem Verhältnis zwischen Masse $m$ und Volumen $V=\frac{4 \pi}{3} \left(\frac{d}{2}\right)^3$, d.h. $\rho = \frac{m}{V}$.
Die Zimmertemperatur betrug stets $T_{Zt}=\qty[]{19}{\degreeCelsius}$.

\begin{table}
    \caption[]{Kenngrößen der kleinen und der großen Kugel}
    \label{tab:kenngroessen}
    \centering
    \begin{tabular}[]{S S[table-format=1.4] S[table-format=2.2] @{${}\pm{}$} S[table-format=0.2] S[table-format=1.4]}
        \toprule
        {Kugel} & {Masse $m / \unit{\kg}$} & \multicolumn{2}{c}{Durchmesser $d / \unit{\mm}$}  & {Dichte $\rho / \left( \unit{\mg \per \cubic\mm}\right)$} \\
        \midrule
...
Für die kleine Kugel oben ergibt sich mit Tabelle \ref{tab:klKu_Zitemp} also für die Fallzeit ein arithmetisches Mittel von 
\begin{align*}
    \overline{t}_{oben} = \frac{1}{10} \sum_{k=1}^{10} {t_{oben, k}} = \qty[]{12.324}{\s}
\end{align*}
mit einer Standardabweichung von 
\begin{align*}
    \Delta \overline{t}_{oben} = 
    \sqrt[]{\frac{1}{N \left(N-1\right)} \sum_{k=1}^{10} \left(t_{oben, k} - \overline{t}_{oben}\right)^2}
    = \qty[]{0.155}{\s}.
\end{align*}

Analog folgt für unten eine Fallzeit von der Mittelwert
\begin{align*}
    \overline{t}_{unten} = \qty[]{12.248}{\s}
\end{align*}
und die Standardabweichung
\begin{align*}
    \overline{t}_{unten} = \qty[]{0.18405}{\s}.
\end{align*}
...
\subsection[]{Messreihe 2: goße Kugel, konstante Temperatur}
...
\begin{table}
    ...
    \end{tabular}
\end{table}
 %Durchschnittszeiten gross Kugel:
